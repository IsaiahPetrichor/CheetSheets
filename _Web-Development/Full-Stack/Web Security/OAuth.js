...
When it comes to securing systems, there are two key concepts that come up often: authentication and authorization.
...
An image showing a login screen that says "Sign in to GitHub to continue to Codecademy" and asks for a username and password.
...
Codecademy will request an access token from the GitHub API, while will provide the authorization code and other authentication details in a POST request.
...
An image showing the back and forth steps between the consumer and service provider in OAuth before access is granted to the consumer.
...
An image showing an example access token
...
Diagram comparing Client Credentials, Authorization Code and Device Code grant types
...
PKCE is an extension to the Authorization Code flow, and it used to prevent attacks and to securely perform the OAuth exchange from public clients. This extension helps prevent authorization code injection from malicious actors.
...
Threats to OAuth
OAuth tokens are great at defending users against data breaches because even if websites that use OAuth to log in on are hacked. There are no passwords contained in databases, and, even if an attacker were to obtain the access tokens, the usually short-lived nature of the access tokens means an attacker would not be able to do much with them.
...
If a user grants this access to the malicious application, the attacker would have access to all of the user’s private information on Github — without ever knowing their password! This was the method successfully used by the hacking group Fancy Bear (also known as APT28 and Pawn Storm) to attack the Democratic National Convention in 2016.
